...
!!! summary "Purpose of this page"
    A technical review guide for cloud architects, platform engineers, and senior reviewers. It maps AzAWSLab to the lifecycle domains used throughout Engineering Deep Dive, with links to engineering notes, configuration evidence, and public repository evidence.

## Review framework

The platform is organised across six lifecycle domains. Each domain has dedicated engineering pages that document design decisions, implementation patterns, and evidence routes. This pathway gives reviewers a structured validation path without duplicating the detailed evidence maps in Engineering Deep Dive.
...
| Microsoft Graph PowerShell operations | [Graph and PowerShell Operations](/engineering/graph-powershell-operations/) | Release 1 scripts, Graph consent, and operational output evidence |
...
- Verify that identity scope, access controls, and endpoint compliance are represented by evidence, not narrative only.
- Inspect how Microsoft 365 service operations connect to identity and endpoint controls.
- Confirm that Microsoft Graph PowerShell operations are repeatable and supported by repository evidence.
...
- Verify why Terraform roots are separated and what risk the boundary limits.
...
- Check that delivery governance is presented as architecture rather than operator-local automation.
...
- Inspect BGP, IPSec, and AWS branch validation as part of the multi-cloud routing claim.
...
- Review how AVD supports controlled administration rather than a generic remote desktop.
...
| Operations automation and runbook execution | [Automation Control Plane](/engineering/automation-control-plane/) | Ansible, AWX, job-template, and execution evidence |
...
- Review how runbooks and automation execution are governed and routed to evidence.
...
- Verify that AI-assisted operations are bounded by policy-mediated tool use and human approval boundaries.
...
3. Use each engineering page's evidence map to locate repository evidence.
