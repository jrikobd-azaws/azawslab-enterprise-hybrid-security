...
!!! summary "Scope"
    State-boundary notes and evidence paths for the Release 2 Terraform split-state architecture, including root ownership, pipeline boundaries, and remote state design. The design limits blast radius, keeps review scope bounded, and routes each platform lifecycle to a clear Terraform root.

## State boundary model

Release 2 uses a split-state Terraform model rather than one monolithic root.

Each root owns a distinct platform lifecycle boundary. This keeps network changes, platform services, governance controls, AWS branch resources, and workload resources from being planned and applied as one shared unit.

| Design choice | Engineering rationale | Evidence path |
|---|---|---|
| Split Terraform roots by operational lifecycle | Limits blast radius and allows each platform area to be planned, reviewed, and applied independently. | [Terraform state and pipeline map](https://github.com/jrikobd-azaws/azawslab-enterprise-hybrid-security/blob/main/docs/release2/11-terraform-state-and-pipeline-map.md) |
...
| Use a dedicated state boundary per active root | Prevents unrelated platform domains from sharing one state file or one state ownership model. | [Terraform root directory](https://github.com/jrikobd-azaws/azawslab-enterprise-hybrid-security/tree/main/terraform) |
...
The repository separates platform ownership across multiple Terraform roots. The engineering signal is that state ownership follows platform responsibility, not only that Terraform exists.
...
| `platform-avd/dev` | Secure administrative workspace | AVD host pool, workspace, session hosts, FSLogix storage, and private access orientation. | [Terraform roots](https://github.com/jrikobd-azaws/azawslab-enterprise-hybrid-security/tree/main/terraform) |
...
| `aws-branch/dev` | AWS multi-cloud edge | AWS branch routing pattern, Transit Gateway, Cisco CSR 8000V, BGP configuration, and route-map integration. | [Terraform roots](https://github.com/jrikobd-azaws/azawslab-enterprise-hybrid-security/tree/main/terraform) |

The retired root `environments/dev-retired-after-state-split` remains as traceability evidence for the earlier state model. It documents the platform's evolution from a simpler root layout into the current split-state architecture.

## Remote state design

Release 2 stores Terraform state remotely in Azure Storage. Each active root has its own backend boundary, using a dedicated state container or state path so one root does not share state ownership with another.
...
This keeps the state layer from becoming an invisible coupling mechanism between networking, governance, AKS, AVD, workloads, management, shared services, and AWS branch resources.

## Isolation in practice

Split state is a delivery-control decision, not a folder layout preference.
...
The key control is that the pipeline is designed around explicit root scope. A networking change is reviewed as a networking plan. An AVD change is reviewed as an AVD plan. An AWS branch change is reviewed as an AWS branch plan.

## Active profile guardrails

Some roots include optional or cost-sensitive resources. The network and AWS branch roots are examples because they can include VPN Gateway, FortiGate, Cisco 8000V, route tables, public IPs, validation infrastructure, and other resources that must not be accidentally created, destroyed, or replaced.

The engineering rule is direct:
...
The current implementation keeps state remote, scoped, and outside the repository. In a stricter enterprise environment, the same pattern can be extended with additional controls around the state storage plane:
...
Terraform state can contain sensitive resource metadata. Treating the backend as a control-plane asset is part of the platform design, not a background implementation detail.

## Engineering significance

Terraform State Boundaries shows six delivery engineering decisions:
...
5. Plan-review-apply separation gives reviewers a concrete control point before infrastructure mutation.
...
This separates Terraform as a platform control plane from Terraform as a single deployment script.
...
| AWS branch resources have their own Terraform root | [Terraform root directory](https://github.com/jrikobd-azaws/azawslab-enterprise-hybrid-security/tree/main/terraform) | `aws-branch/dev` and its AWS branch lifecycle scope |
...
| Enterprise state hardening pattern is documented | This page | Remote state treated as a protected control-plane asset, with optional CMK, private endpoint, firewall, RBAC, versioning, soft delete, and immutable retention patterns documented |
