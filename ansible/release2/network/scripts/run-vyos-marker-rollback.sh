...
: "${A2_ROLLBACK_APPROVED:=false}"
: "${A2_CHANGE_TICKET:=}"
: "${A2_ROLLBACK_REASON:=}"

if [ "$A2_ROLLBACK_APPROVED" != "true" ]; then
  echo "ERROR: A2_ROLLBACK_APPROVED must be true for Tier 5 rollback." >&2
  exit 1
fi

if [ -z "$A2_CHANGE_TICKET" ]; then
  echo "ERROR: A2_CHANGE_TICKET is required for Tier 5 rollback." >&2
  exit 1
fi
...
if [ -z "$A2_ROLLBACK_REASON" ]; then
  echo "ERROR: A2_ROLLBACK_REASON is required for Tier 5 rollback." >&2
  exit 1
fi
...
ansible-playbook \
  ...
  playbooks/vyos-marker-rollback.yml
...
echo "A2 VyOS marker rollback complete. Runtime secrets unset."